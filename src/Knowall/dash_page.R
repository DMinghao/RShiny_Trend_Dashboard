...
twitterLayout <- div(tabsetPanel(
  tabPanel(
    "Result", icon = icon("fas fa-poll-h"), 
    box(p("Below is the example of the word cloud, it presents the tweets related to three existing keywords that are selected defaultly. You could change and enter any other keywords that you are interested in and use the visualizations for further analysis. ")), 
    box(p("In the first word cloud, the larger the font, the more 'likes' the tweet has.The specific tweets are illustrated in the table below. It is sorted by the 'number of likes' to rank in order, and the higher the ranking, the higher the 'number of likes' for the tweet.")),
    ...
    box(p("NOTICE*:If only part of the content is displayed, it means that the amount of single-day or single-week tweets related to the keyword is beyond the scope of the API, indicating that the keyword has been discussed in a very large amount recently.")), 
    ...
    box(p("This is the sentiment analysis graph of twitter The arc represents the degree of emotion. Through the time on the horizontal axis, you can observe the emotion involved in the keyword in the time dimension.")), 
